...
const guard = Object.values(guards)
	.sort((a, b) => b.mostAsleepMinuteTimes - a.mostAsleepMinuteTimes)
	[0]
...
function readLog(log) {
	...
	Object.values(guards).forEach(guard => {
		...
		guard.mostAsleepMinuteTimes = Math.max(...guard.minutes);
		guard.mostAsleepMinute = guard.minutes.indexOf(guard.mostAsleepMinuteTimes);
	});
