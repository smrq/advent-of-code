...
console.log(run(boardString, 23, true));

function run(boardString, atk, DEBUG) {
	const state = parseBoardString(boardString, atk);
...
function parseBoardString(str, atk) {
	const units = [];
	const board = str
		.split('\n')
		.map((line, y) => line
			...
			.map((type, x) => {
				switch (type) {
					...
					case '.':
						return emptySpace;
					case 'E': {
						const unit = { type, hp: 200, atk, coords: `${x},${y}` }
						units.push(unit);
						return unit;
					}
					case 'G': {
