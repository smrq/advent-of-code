...
function parseInput(input) {
	...
	for (let arr of Object.values(points)) {
		...
		if (a[0] === 2 || a[1] === 2 || a[0] === 132 || a[1] === 132) {
			teleports[a.join(',')] = [...b, -1];
			teleports[b.join(',')] = [...a, 1];
		} else {
			teleports[a.join(',')] = [...b, 1];
			teleports[b.join(',')] = [...a, -1];
		}
...
async function run(input) {
	...
	const start = [...points.AA[0], 0];
	const goal = [...points.ZZ[0], 0];
	...
	return astar({
		...
		neighbors: ([x, y, z]) => {
			...
			if (teleports[`${x},${y}`]) {
				const [tx, ty, dz] = teleports[`${x},${y}`];
				if (z + dz >= 0) {
					result.push([tx, ty, z + dz]);
				}
			}
			return result;
		},
