...
async function run(input) {
	const debug = false;
	const { grid, center } = exploreMap(input, debug);	
...
function exploreMap(input, debug) {
	...
	while (openSet.length) {
		const { machine, path, coords } = openSet.shift();
		...
		for (let move of nextDirections(path)) {
			const newCoords = updateCoords(coords, move);
			if (grid.has(newCoords.join(','))) {
				continue;
			}
			...
			if (result.value !== 0) {
				grid.set(newCoords.join(','), result.value);
				openSet.push({
					path: [...path, move],
					machine: newMachine,
					coords: newCoords
