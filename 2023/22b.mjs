...
function calculateFalls(input, i) {
	const supported = input.map(brick => new Set(brick.supported));
	const onGround = supported.filter(set => set.size === 0).length;
	...
	return supported.filter(set => set.size === 0).length - onGround;
}
...
function run(input) {
	...
	return L.sum(input.map((_, i) => {
		return calculateFalls(input, i);
	}));
}
