...
function scoreHand(hand) {
	...
	const wildCount = countCards(hand, 'J');

	// Five of a kind
	if (wildCount === 5 || cards[0].count + wildCount === 5) {
		return [7, ...subrank];
	}

	// Four of a kind
	if (cards[0].count + wildCount === 4) {
		return [6, ...subrank];
	}

	// Full house
	if (cards[0].count + cards[1].count + wildCount === 5) {
		return [5, ...subrank];
	}

	// Three of a kind
	if (cards[0].count + wildCount === 3) {
		return [4, ...subrank];
	}

	// Two pair
	if (
		(cards[0].count + cards[1].count + wildCount === 4) &&
		(cards[1].count + wildCount >= 2)
	) {
		return [3, ...subrank];
	}

	// One pair
	if (cards[0].count + wildCount === 2) {
		return [2, ...subrank];
	}
